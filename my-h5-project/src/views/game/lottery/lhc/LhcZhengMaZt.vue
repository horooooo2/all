<template>
  <div class="lhc-zmt" :aria-label="config.mainLabel">
    <div class="lhc-zmt__mode" role="tablist" :aria-label="`${config.mainLabel}玩法`">
      <div class="lhc-zmt__mode-grid">
        <button
          v-for="m in config.modes"
          :key="m.key"
          type="button"
          role="tab"
...
import { getZhengMaZtConfig } from './lhc-zhengma-zt-config.js'
...
const config = computed(() => getZhengMaZtConfig(props.variant))
const PLAY_MODES = computed(() => config.value.modes)
...
const picks = ref(initModePickSets(PLAY_MODES.value.map((m) => m.key)))

watch(
  () => props.variant,
  () => {
    subMode.value = 'z1'
    picks.value = initModePickSets(PLAY_MODES.value.map((m) => m.key))
...
const currentMode = computed(() => PLAY_MODES.value.find((m) => m.key === subMode.value) ?? PLAY_MODES.value[0])
...
function getBasketDraft() {
  ...
  const { mainLabel, play } = config.value
  return {
    betCount: betCount.value,
    leftLabel: `${mainLabel} ${modeTitle.value} ${parts.join(',')}`,
    raw: { play, subMode: subMode.value, modeTitle: modeTitle.value, picks: [...set] }
  }
}
