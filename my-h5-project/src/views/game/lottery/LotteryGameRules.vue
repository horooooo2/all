...
import { resolveLotteryGameRulesContentCode } from '@/utils/lotteryGameRulesNavigate'
...
const load = async () => {
  const code = resolveLotteryGameRulesContentCode(route.query.gameId, route.query.gameName)
  if (!code) {
    html.value = ''
    detailTitle.value = ''
    loading.value = false
