...
/** 演示数据见 lottery-bet-records.mock.js；接接口后改为请求结果 */
...
function goBack() {
  ...
  else router.push({ name: 'gameRoom' })
}
...
/* —— 与 my-rebate.less 中 .record-header / .back-btn 一致，并增加右侧筛选 —— */
.record-header {
  height: 44px;
...
/* —— 与 my-rebate.less 中 .top-tabs / .top-tab 一致（使用 div 实现） —— */
.top-tabs {
  flex-shrink: 0;
  height: 48px;
  background: var(--bg-module);
...
<style lang="less">
/* van-popup teleport 到 body 时，与游戏记录同款时间弹层需全局样式 */
